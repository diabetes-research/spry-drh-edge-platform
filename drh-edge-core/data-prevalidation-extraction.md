...
export PORT=9227
export REMOTE_BASE_URL="https://raw.githubusercontent.com/diabetes-research/spry-drh-edge-platform/refs/heads/ani/feat/align-prevalidation-sql-first/drh-edge-core/"
```
...
- The YAML header at the top of this `data-prevalidation-extraction.md` reads `database_url: ${env.SPRY_DB}` and `port: ${env.PORT}` — Spry and the SQLPage tooling will substitute those environment values when building or serving the site.
...
- Place the study data files in a **directory** in the same path as this markdown, then run the following command:
  - ` spry rb task prepare-db-deploy-server  data-prevalidation-extraction.md `
...
```bash process-study-data -I --dep prepare-env --descr "Another demo task"
#!/usr/bin/env -S bash
echo "process-study-data successful"
$!{await partial("prepare-db-deploy-server", { newLocal:{
"STUDY_DATA_PATH":"raw-data/dexcom-synthetic-cgm/",
"TENANT_ID":"DSG",
"TENANT_NAME":"DSG",
...
```bash PARTIAL prepare-db-deploy-server -deploy-server { newLocal: { type: "object", required: true } } --descr "Performs pre-etl-validation , Ingestion, ETL and Server Deployment"
#!/bin/bash
set -u
# Variables
# 1. Cleanup
...
surveilr ingest files -r "${newLocal.STUDY_DATA_PATH}" --tenant-id "${newLocal.TENANT_ID}" --tenant-name "${newLocal.TENANT_NAME}"
surveilr shell --engine duckdb "${newLocal.REMOTE_BASE_URL}/duckdb-etl-sql/drh-preflight-validation.sql"
...
if [ "$VALIDATION_STATUS" == "PASS" ]; then    
    (
        ...
        surveilr shell "${newLocal.REMOTE_BASE_URL}/common-sql/drh-data-validation.sql"    
        surveilr shell "${newLocal.REMOTE_BASE_URL}/common-sql/drh-anonymize-prepare.sql"          
        surveilr shell --engine duckdb "${newLocal.REMOTE_BASE_URL}/duckdb-etl-sql/drh-master-etl.sql"
        surveilr shell "${newLocal.REMOTE_BASE_URL}/common-sql/drh-metrics-pipeline.sql" 
    )
...
spry sp spc --package --conf sqlpage/sqlpage.json -m data-prevalidation-extraction.md | sqlite3 resource-surveillance.sqlite.db
```

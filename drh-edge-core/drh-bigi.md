...
- back to sqlite db to be used in SQLpage
...
```envrc prepare-env -C ./.env --gitignore -X  --descr "Generate .envfile and add it to local .gitignore if it's not already there"
OTEL_SERVICE_NAME="tap-bigi" 
OTEL_SERVICE_VERSION="1.0.0"
STUDY_DATA_PATH="raw-data/dexcom-bigi-cgm/"
TENANT_ID="DUKE"
TENANT_NAME="Duke University"
```
...
surveilr ingest files -r singer-tap/tap-bigi.surveilr\[singer\].py
...
JSON_OBJECT(
    'ambulatoryGlucoseProfile', (
        ...
        percentiles AS (
            SELECT
                hourValue AS hour,
                -- Use MAX(CASE) to collapse the windowed rows into a single hour row
        ...
        -- This creates the flat array your JavaScript expects
...
SELECT 'json' AS component, 
JSON_OBJECT('timeMetrics', (
    SELECT JSON_OBJECT(
        'participant_id', participant_id, 
        'timeBelowRangeVeryLow', ROUND(SUM(CASE WHEN CGM_Value < 54 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)),
        'timeBelowRangeLow',     ROUND(SUM(CASE WHEN CGM_Value BETWEEN 54 AND 69 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)),
        'timeInRange',           ROUND(SUM(CASE WHEN CGM_Value BETWEEN 70 AND 180 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)),
        'timeAboveRangeHigh',    ROUND(SUM(CASE WHEN CGM_Value BETWEEN 181 AND 250 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)),
        'timeAboveRangeVeryHigh',ROUND(SUM(CASE WHEN CGM_Value > 250 THEN 1 ELSE 0 END) * 100.0 / COUNT(*))
    )
    FROM combined_cgm_tracing_cached
    WHERE participant_id = $participant_id AND Date_Time BETWEEN $start_date AND $end_date
)) AS contents;
```
...
SELECT 'json' AS component, 
JSON_OBJECT('glycemicRiskIndicator', (
    WITH raw_pct AS (
        SELECT 
            SUM(CASE WHEN CGM_Value < 54 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) as vlow,
            SUM(CASE WHEN CGM_Value BETWEEN 54 AND 69 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) as low,
            SUM(CASE WHEN CGM_Value > 250 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) as vhigh,
            SUM(CASE WHEN CGM_Value BETWEEN 181 AND 250 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) as high,
            SUM(CASE WHEN CGM_Value BETWEEN 70 AND 180 THEN 1 ELSE 0 END) * 100.0 / COUNT(*) as tir
        FROM combined_cgm_tracing_cached
        WHERE participant_id = $participant_id AND Date_Time BETWEEN $start_date AND $end_date
    )
    SELECT JSON_OBJECT(
        'time_below_VL_percentage', ROUND(COALESCE(vlow, 0), 2),
        'time_below_low_percentage', ROUND(COALESCE(low, 0), 2),
        'time_in_range_percentage', ROUND(COALESCE(tir, 0), 2),
        'time_above_H_percentage', ROUND(COALESCE(high, 0), 2),
        'time_above_VH_percentage', ROUND(COALESCE(vhigh, 0), 2),
        'Hypoglycemia_Component', ROUND(COALESCE(vlow + (0.8 * low), 0), 2),
        'Hyperglycemia_Component', ROUND(COALESCE(vhigh + (0.5 * high), 0), 2),
        'GRI', ROUND(COALESCE((3.0 * (vlow + (0.8 * low))) + (1.6 * (vhigh + (0.5 * high))), 0), 2)
    ) FROM raw_pct
)) AS contents;
```
...
WITH stats AS (
    SELECT 
        ...
        COUNT(DISTINCT DATE(Date_Time)) as days_worn,
        AVG(CGM_Value) as mean_g,
        (SQRT(AVG(CGM_Value * CGM_Value) - (AVG(CGM_Value) * AVG(CGM_Value))) / AVG(CGM_Value)) * 100 as cv_val,
        (julianday(MAX(Date_Time)) - julianday(MIN(Date_Time)) + 1) as total_range_days
    FROM combined_cgm_tracing_cached
    WHERE participant_id = $participant_id 
      AND Date_Time BETWEEN $start_date AND $end_date
),
-- 2. Fetch Formulas from metric_definitions
defs AS (
    SELECT 
        metric_id, 
        metric_name,
        json_extract(metric_info, '$.description') as description
    FROM metric_definitions
    WHERE metric_id IN ('time_cgm_active', 'number_of_days_cgm_worn', 'mean_glucose', 'gmi', 'glucose_variability')
)
SELECT 
    -- Layout Start
    '<div class="fs-3 p-1 fw-bold" style="background-color: #E3E3E2; text-black;">GLUCOSE STATISTICS AND TARGETS</div><div class="px-4">' ||
    
    -- Date Range Bar
    '<div class="card-content my-1">'|| strftime('%Y-%m-%d', s.min_dt) || ' - ' || strftime('%Y-%m-%d', s.max_dt) || 
    ' <span style="float: right;">'|| CAST(s.total_range_days - 1 AS INTEGER) ||' days</span></div>' ||

    -- Time CGM Active
    '<div class="card-content my-1" style="display: flex; flex-direction: row; justify-content: space-between;"><b>Time CGM Active</b> 
        <div style="display: flex; justify-content: flex-end; align-items: center;">
            <div style="display: flex;align-items: center;gap: 0.1rem;"><b>' || 
            ROUND((s.days_worn / s.total_range_days) * 100, 2) || '</b> % 
            <formula-component content="' || (SELECT description FROM defs WHERE metric_id = 'time_cgm_active') || '"></formula-component>
        </div></div>
    </div>' ||

    -- Number of Days
    '<div class="card-content my-1" style="display: flex; flex-direction: row; justify-content: space-between;"><b>Number of Days CGM Worn</b> 
        <div style="display: flex; justify-content: flex-end; align-items: center;">
            <div style="display: flex;align-items: center;gap: 0.1rem;"><b>'|| s.days_worn ||'</b> days
            <formula-component content="' || (SELECT description FROM defs WHERE metric_id = 'number_of_days_cgm_worn') || '"></formula-component>
        </div></div>
    </div>' ||

    -- Target Table (Keeping your exact HTML structure)
    '<div class="card-body" style="background-color: #E3E3E2;border: 1px solid black;">
        <div class="table-responsive">
            <table class="table">
                <tbody class="table-tbody list">
                    <tr><th colspan="2" style="text-align: center;">Ranges And Targets For Type 1 or Type 2 Diabetes</th></tr>
                    <tr><th>Glucose Ranges</th><th>Targets % of Readings (Time/Day)</th></tr>
                    <tr><td>Target Range 70-180 mg/dL</td><td>Greater than 70% (16h 48min)</td></tr>
                    <tr><td>Below 70 mg/dL</td><td>Less than 4% (58min)</td></tr>
                    <tr><td>Below 54 mg/dL</td><td>Less than 1% (14min)</td></tr>
                    <tr><td>Above 180 mg/dL</td><td>Less than 25% (6h)</td></tr>
                    <tr><td>Above 250 mg/dL</td><td>Less than 5% (1h 12min)</td></tr>
                    <tr><td colspan="2">Each 5% increase in time in range (70-180 mg/dL) is clinically beneficial.</td></tr>
                </tbody>
            </table>
        </div>
    </div>' ||
    ...
    -- Mean Glucose
    '<div class="card-content my-1" style="display: flex; flex-direction: row; justify-content: space-between;"><b>Mean Glucose</b> 
        <div style="display: flex; justify-content: flex-end; align-items: center;">
            <div style="display: flex;align-items: center;gap: 0.1rem;"><b>'|| ROUND(s.mean_g, 2) ||'</b> mg/dL
            <formula-component content="' || (SELECT description FROM defs WHERE metric_id = 'mean_glucose') || '"></formula-component>
        </div></div>
    </div>' ||

    -- GMI
    '<div class="card-content my-1" style="display: flex; flex-direction: row; justify-content: space-between;"><b>GMI</b> 
        <div style="display: flex; justify-content: flex-end; align-items: center;">
            <div style="display: flex;align-items: center;gap: 0.1rem;"><b>'|| ROUND(s.mean_g * 0.155 + 95, 2) ||'</b> %
            <formula-component content="' || (SELECT description FROM defs WHERE metric_id = 'gmi') || '"></formula-component>
        </div></div>
    </div>' ||

    -- Glucose Variability
    '<div class="card-content my-1" style="display: flex; flex-direction: row; justify-content: space-between;"><b>Glucose Variability</b> 
        <div style="display: flex; justify-content: flex-end; align-items: center;">
            <div style="display: flex;align-items: center;gap: 0.1rem;"><b>'|| ROUND(s.cv_val, 2) ||'</b> %
            <formula-component content="' || (SELECT description FROM defs WHERE metric_id = 'glucose_variability') || '"></formula-component>
        </div></div>
    </div>' ||
...
WITH base_data AS (
    SELECT 
        CGM_Value,
        Date_Time,
        DATE(Date_Time) as d_date,
        LAG(CGM_Value) OVER (ORDER BY Date_Time) AS lag_cgm,
        AVG(CGM_Value) OVER () as global_mean,
        (AVG(CGM_Value * CGM_Value) OVER () - POWER(AVG(CGM_Value) OVER (), 2)) as global_variance
    FROM combined_cgm_tracing_cached
    WHERE participant_id = $participant_id 
...
calc_metrics AS (
    SELECT
        ROUND(CAST(SUM(CASE WHEN CGM_Value < 70 OR CGM_Value > 180 THEN 1 ELSE 0 END) AS REAL) / COUNT(*), 2) as liability_index,
        SUM(CASE WHEN CGM_Value < 70 THEN 1 ELSE 0 END) as hypo_episodes,
        SUM(CASE WHEN CGM_Value BETWEEN 70 AND 180 THEN 1 ELSE 0 END) as eugly_episodes,
        SUM(CASE WHEN CGM_Value > 180 THEN 1 ELSE 0 END) as hyper_episodes,
        ROUND((MAX(CGM_Value) - MIN(CGM_Value)) / NULLIF((strftime('%s', MAX(Date_Time)) - strftime('%s', MIN(Date_Time))) / 60.0, 0), 3) as m_value,
        ROUND(0.001 * POWER(MAX(global_mean) + SQRT(MAX(global_variance)), 2), 2) as j_index,
        ROUND(AVG(CASE
            WHEN CGM_Value < 90 THEN 10 * POWER(5 - (CGM_Value / 18.0), 2)
            WHEN CGM_Value > 180 THEN 10 * POWER((CGM_Value / 18.0) - 10, 2)
            ELSE 0 END), 3) as grade,
        ROUND(SQRT(AVG(POWER(CGM_Value - lag_cgm, 2))), 3) as conga,
        -- LBGI and HBGI Calculations
        ROUND(SUM(CASE WHEN CGM_Value < 70 THEN POWER(1.509 * (POWER(LOG(CGM_Value/18.0), 1.084) - 5.381), 2) ELSE 0 END) / COUNT(*), 2) as lbgi,
        ROUND(SUM(CASE WHEN CGM_Value > 180 THEN POWER(1.509 * (POWER(LOG(CGM_Value/18.0), 1.084) - 5.381), 2) ELSE 0 END) / COUNT(*), 2) as hbgi
    FROM base_data
),
daily_metrics AS (
    SELECT 
        AVG(max_g - min_g) as adrr,
        AVG(ABS(max_g - min_g)) as mean_amplitude,
        AVG(ABS(daily_diff)) as modd
    FROM (
        SELECT 
            d_date, 
            MAX(CGM_Value) as max_g, 
            MIN(CGM_Value) as min_g,
            MAX(CGM_Value) - LAG(MAX(CGM_Value)) OVER (ORDER BY d_date) as daily_diff
        FROM base_data 
        GROUP BY d_date
    )
)
SELECT 
    m.metric_name as label,
    CASE 
        WHEN m.metric_id = 'liability_index' THEN c.liability_index || ' mg/dL'
        WHEN m.metric_id = 'hypoglycemic_episodes' THEN c.hypo_episodes || ''
        WHEN m.metric_id = 'euglycemic_episodes' THEN c.eugly_episodes || ''
        WHEN m.metric_id = 'hyperglycemic_episodes' THEN c.hyper_episodes || ''
        WHEN m.metric_id = 'm_value' THEN c.m_value || ' mg/dL'
        WHEN m.metric_id = 'mean_amplitude' THEN ROUND(d.mean_amplitude, 3) || ' mg/dL'
        WHEN m.metric_id = 'average_daily_risk_range' THEN ROUND(d.adrr, 3) || ' mg/dL'
        WHEN m.metric_id = 'j_index' THEN c.j_index || ' mg/dL'
        WHEN m.metric_id = 'low_blood_glucose_index' THEN c.lbgi || ''
        WHEN m.metric_id = 'high_blood_glucose_index' THEN c.hbgi || ''
        WHEN m.metric_id = 'glycemic_risk_assessment' THEN c.grade || ''
        WHEN m.metric_id = 'continuous_overall_net_glycemic_action' THEN c.conga || ''
        WHEN m.metric_id = 'mean_of_daily_differences' THEN ROUND(d.modd, 3) || ''
    END as value,
    JSON_EXTRACT(m.metric_info, '$.formula') as formula
FROM calc_metrics c, daily_metrics d
JOIN metric_definitions m ON m.metric_id IN (
    'liability_index', 'hypoglycemic_episodes', 'euglycemic_episodes', 
    'hyperglycemic_episodes', 'm_value', 'mean_amplitude', 
    'average_daily_risk_range', 'j_index', 'low_blood_glucose_index', 
    'high_blood_glucose_index', 'glycemic_risk_assessment', 'continuous_overall_net_glycemic_action', 'mean_of_daily_differences'
)
ORDER BY m.rowid; -- Keeps the order matching your table
```

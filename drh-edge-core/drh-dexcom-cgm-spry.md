...
- The YAML header at the top of this `drh-dexcom-cgm-spry.md` reads `database_url: ${env.SPRY_DB}` and `port: ${env.PORT}` — Spry and the SQLPage tooling will substitute those environment values when building or serving the site.
...
- Place the study data files in a **directory** in the same path as this markdown, then run the following command:
  - ` ./spry.ts task -m drh-dexcom-cgm-spry.md prepare-db `
...
./spry.ts spc -m drh-dexcom-cgm-spry.md --fs dev-src.auto --destroy-first --conf sqlpage/sqlpage.json  
```
...
whenever you update `drh-dexcom-cgm-spry.md`, it regenerates the SQLPage `dev-src.auto`,
which is then picked up automatically by the SQLPage server:

```bash
./spry.ts spc -m drh-dexcom-cgm-spry.md --fs dev-src.auto --destroy-first --conf sqlpage/sqlpage.json --watch --with-sqlpage
```
...
```bash
./spry.ts spc -m drh-dexcom-cgm-spry.md --fs dev-src.auto --destroy-first --conf sqlpage/sqlpage.json --watch
```

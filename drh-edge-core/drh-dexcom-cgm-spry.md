...
- Place the study data files in a **directory** in the same path as this markdown, then run the following command:
  - ` spry rb task prepare-db drh-dexcom-cgm-spry.md `
...
```bash
spry sp spc  --fs dev-src.auto --destroy-first --conf sqlpage/sqlpage.json --watch --with-sqlpage -m drh-dexcom-cgm-spry.md 
```
...
```bash
spry sp spc  --fs dev-src.auto --destroy-first --conf sqlpage/sqlpage.json --watch -m drh-dexcom-cgm-spry.md 
```

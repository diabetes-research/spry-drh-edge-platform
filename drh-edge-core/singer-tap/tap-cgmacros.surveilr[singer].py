...
def get_stream_keys():
    ...
    for stream in ALL_STREAM_NAMES:
        ...
        if schema and "key_properties" in schema:
            ...
            # Check if this property exists in schema to be safe
            if (
                schema
                ...
                with open(fpath, "r", encoding="utf-8-sig") as f:
                    ...
                    try:
                        ...
                        if emitter and resource_id and file_start_time:
                            ...
                            req_cols_span_id = str(uuid.uuid4()).replace("-", "")[
                                :16
                            ]  # Ensure ID exists
            ...
            for row in rows:
                ...
                if expected_file:
                    ...
                    if os.path.exists(target_path):
                        ...
                        try:
                            with open(
                                target_path, "r", encoding="utf-8-sig"
                            ) as data_file:
        ...
        try:
            with open(meal_meta_path, "r", encoding="utf-8-sig") as f:
                ...
                if "file_name" not in reader.fieldnames:
                    return [
                        {
                            "name": "Meal Metadata Schema Check",
                            "status": "SKIPPED",
                            "details": "file_name column missing in metadata",
                        }
                    ]
        ...
        try:
            with open(fitness_meta_path, "r", encoding="utf-8-sig") as f:
                ...
                if "file_name" not in reader.fieldnames:
                    return [
                        {
                            "name": "Fitness Metadata Schema Check",
                            "status": "SKIPPED",
                            "details": "file_name column missing in metadata",
                        }
                    ]
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        "WARN",
                        f"Error in cgm_file_metadata file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in publication file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in participant file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in study file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in investigator file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in institution file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in lab file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in meal_file_metadata file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in fitness_file_metadata file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in author file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in meal file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in fitness file {filepath}: {e}",
                        {"file": filepath},
                    )
            ...
            try:
                ...
                if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
                    emit_otel_log(
                        ...
                        emitter.otel_resource_id,
                        "WARN",
                        f"Error in site file {filepath}: {e}",
                        {"file": filepath},
                    )
    ...
    try:
        with open(meta_path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                try:
                    ...
                    if (
                        hasattr(emitter, "otel_resource_id")
                        and emitter.otel_resource_id
                    ):
                        emit_otel_log(
                            emitter,
                            emitter.otel_resource_id,
                            "WARN",
                            f"Error processing CGM data file {file_name}: {e}",
                            {"file": file_name},
                        )
        ...
        if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
            emit_otel_log(
                ...
                emitter.otel_resource_id,
                "ERROR",
                f"Error processing combined CGM tracing: {e}",
            )
    ...
    try:
                ...
                try:
                    ...
                    if (
                        ...
                        emit_otel_log(
                            ...
                            emitter.otel_resource_id,
                            "WARN",
                            f"Error processing raw meal file {file_name}: {e}",
                            {"file": file_name},
                        )
        ...
        if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
            emit_otel_log(
                ...
                emitter.otel_resource_id,
                "ERROR",
                f"Error processing raw meal metadata: {e}",
            )
    ...
    try:
        with open(meta_path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                try:
                    ...
                    LOGGER.warning(
                        f"Error processing raw fitness file {file_name}: {e}"
                    )
                    if (
                        hasattr(emitter, "otel_resource_id")
                        and emitter.otel_resource_id
        ...
        if hasattr(emitter, "otel_resource_id") and emitter.otel_resource_id:
            emit_otel_log(
                ...
                emitter.otel_resource_id,
                "ERROR",
                f"Error processing raw fitness metadata: {e}",
            )
...
def main():
    ...
    check_counter = (
        1  # Not used for ID anymore, strictly for internal counting if needed
    )
    diagnostic_logs = []
    ...
    def emit_diagnostic(check_id_static, name, status, details):
        ...
        diagnostic_logs.append({"check": name, "status": status, "details": details})
        check_counter += 1

    def fail_and_exit(stage_name, errors_count=1):
        """Helper to emit failure report and exit immediately."""
        LOGGER.error(
            f"Validation failed at stage: {stage_name}. See drh_diagnostics for details."
        )
        report_record = {
            "timestamp": timestamp,
            "folder_name": (
                os.path.basename(os.path.normpath(data_dir)) if data_dir else "unknown"
            ),
        ...
        emit_otel_log(
            ...
            {"folder": data_dir, "errors": str(errors_count)},
    ...
    if not os.path.exists(data_dir):
        ...
        emit_otel_log(
            ...
            otel_resource_id,
            "ERROR",
            f"Diagnostic FAILED: Folder & Resource Check",
            attributes={"check_id": 1, "details": msg},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
        ...
        emit_otel_span(
            ...
            span_id=folder_span_id,
            trace_id=root_trace_id,
            attributes={
                OTelNames.ATTR_VALIDATION_LEVEL: "folder",
        ...
        file_count = sum(
            1
            for _ in glob.glob(os.path.join(data_dir, "**", "*"), recursive=True)
            if os.path.isfile(_)
        )
        ...
        emit_otel_log(
            ...
            otel_resource_id,
            "INFO",
            f"Diagnostic PASSED: Folder & Resource Check",
            attributes={"check_id": 1, "details": msg},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
        ...
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_PASS_COUNT,
            1,
            attributes={"check.category": OTelNames.CAT_FOLDER_SCAN},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_FILE_COUNT,
            file_count,
            attributes={"check.category": OTelNames.CAT_FOLDER_SCAN},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_VALIDATION_DURATION,
            (get_time_nano() - folder_val_start) / 1e9,
            unit="s",
            attributes={"check.category": OTelNames.CAT_FOLDER_SCAN},
        )
    ...
    if missing:
        ...
        emit_otel_log(
            ...
            otel_resource_id,
            "ERROR",
            f"Diagnostic FAILED: Mandatory File Presence",
            attributes={"check_id": 2, "details": msg},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
        ...
        emit_otel_span(
            ...
            get_time_nano(),
            parent_span_id=root_span_id,
            span_id=check2_span_id,
            trace_id=root_trace_id,
            attributes={
                OTelNames.ATTR_VALIDATION_LEVEL: "file_presence",
                "missing": str(missing),
            },
            status_code="ERROR",
        )
        ...
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_FAIL_COUNT,
            1,
            attributes={"check.category": OTelNames.CAT_MANDATORY_FILES},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_VALIDATION_DURATION,
            (get_time_nano() - check2_start) / 1e9,
            unit="s",
            attributes={"check.category": OTelNames.CAT_MANDATORY_FILES},
        )
        ...
        emit_diagnostic(
            2, "Mandatory File Presence", "PASSED", "All mandatory files present"
        )
        emit_otel_log(
            emitter,
            otel_resource_id,
            "INFO",
            f"Diagnostic PASSED: Mandatory File Presence",
            attributes={"check_id": 2, "details": "All mandatory files present"},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
        ...
        emit_otel_span(
            ...
            span_id=check2_span_id,
            trace_id=root_trace_id,
            attributes={OTelNames.ATTR_VALIDATION_LEVEL: "file_presence"},
            status_code="OK",
        )
    ...
    if extension_errors:
        for err in extension_errors:
            ...
            emit_otel_log(
                ...
                otel_resource_id,
                "ERROR",
                f"Diagnostic FAILED: File Extension Validation",
                attributes={"check_id": 3, "details": err},
                span_id=current_span_id,
                trace_id=root_trace_id,
            )
        ...
        emit_otel_span(
            ...
            get_time_nano(),
            parent_span_id=root_span_id,
            span_id=check3_span_id,
            trace_id=root_trace_id,
            attributes={
                OTelNames.ATTR_VALIDATION_LEVEL: "extensions",
                OTelNames.ATTR_ERROR_COUNT: len(extension_errors),
            },
            status_code="ERROR",
        )
        ...
        emit_otel_metric(
            ...
            attributes={"check.category": OTelNames.CAT_EXTENSION_CHECK},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_VALIDATION_DURATION,
            (get_time_nano() - check3_start) / 1e9,
            unit="s",
            attributes={"check.category": OTelNames.CAT_EXTENSION_CHECK},
        )
        ...
        emit_otel_log(
            ...
            otel_resource_id,
            "INFO",
            f"Diagnostic PASSED: File Extension Validation",
            attributes={"check_id": 3, "details": msg},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
        ...
        emit_otel_span(
            ...
            span_id=check3_span_id,
            trace_id=root_trace_id,
            attributes={OTelNames.ATTR_VALIDATION_LEVEL: "extensions"},
            status_code="OK",
        )
    ...
    schema_results = check_file_headers(
        data_dir,
        emitter=emitter,
        resource_id=otel_resource_id,
        parent_span_id=schema_span_id,
        trace_id=root_trace_id,
    ...
    for res in schema_results:
        ...
        emit_otel_log(
            ...
            otel_resource_id,
            otel_severity,
            f"Diagnostic {res['status']}: {res['name']}",
            attributes={"check_id": 4, "details": res["details"]},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
    ...
    emit_otel_span(
        ...
        get_time_nano(),
        parent_span_id=root_span_id,
        span_id=schema_span_id,
        trace_id=root_trace_id,
        attributes={
            OTelNames.ATTR_VALIDATION_LEVEL: "schema",
            "files.checked": len(schema_results),
    ...
    emit_otel_metric(
        ...
        OTelNames.METRIC_PASS_COUNT,
        file_pass_count,
        attributes={"check.category": OTelNames.CAT_SCHEMA_VALIDATION},
    )
    emit_otel_metric(
        emitter,
        otel_resource_id,
    ...
    emit_otel_metric(
        ...
        otel_resource_id,
        OTelNames.METRIC_VALIDATION_DURATION,
        (get_time_nano() - schema_val_start) / 1e9,
        unit="s",
        attributes={"check.category": OTelNames.CAT_SCHEMA_VALIDATION},
    )
    ...
    # 4.1.4 Global Foreign Key Validation
    if fk_validator:
        fk_start = get_time_nano()
        fk_span_id = str(uuid.uuid4()).replace("-", "")[:16]
        ...
        if violations:
            ...
            for v in violations:
                ...
                emit_otel_log(
                    ...
                    otel_resource_id,
                    "ERROR",
                    f"Diagnostic FAILED: Foreign Key Check",
                    attributes={"check_id": 4, "details": v},
                    span_id=fk_span_id,
                    trace_id=root_trace_id,
                )
        else:
            ...
            emit_otel_log(
                ...
                otel_resource_id,
                "INFO",
                f"Diagnostic PASSED: Foreign Key Check",
                attributes={"check_id": 4, "details": msg},
                span_id=fk_span_id,
                trace_id=root_trace_id,
            )
        ...
        emit_otel_span(
            ...
            trace_id=root_trace_id,
            attributes={
                OTelNames.ATTR_VALIDATION_LEVEL: "fk_global",
        ...
        if violations:
            fail_and_exit("Foreign Key Integrity", len(violations))
    ...
    for res in consistency_results:
        ...
        emit_otel_log(
            ...
            otel_severity,
            f"Diagnostic {res['status']}: {res['name']}",
            attributes={"check_id": 5, "details": res["details"]},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
    ...
    emit_otel_span(
        ...
        get_time_nano(),
        parent_span_id=root_span_id,
        span_id=check5_span_id,
        trace_id=root_trace_id,
        attributes={
            OTelNames.ATTR_VALIDATION_LEVEL: "cgm_meta",
            "failures": consistency_failures,
        },
        status_code="ERROR" if consistency_failures > 0 else "OK",
    )
    ...
    emit_otel_metric(
        ...
        OTelNames.METRIC_PASS_COUNT,
        passed_count,
        attributes={"check.category": OTelNames.CAT_CGM_TRACE_METADATA},
    )
    emit_otel_metric(
        emitter,
        otel_resource_id,
    ...
    emit_otel_metric(
        ...
        otel_resource_id,
        OTelNames.METRIC_VALIDATION_DURATION,
        (get_time_nano() - check5_start) / 1e9,
        unit="s",
        attributes={"check.category": OTelNames.CAT_CGM_TRACE_METADATA},
    )
    ...
    for res in integrity_results:
        ...
        emit_otel_log(
            ...
            otel_severity,
            f"Diagnostic {res['status']}: {res['name']}",
            attributes={"check_id": 6, "details": res["details"]},
            span_id=current_span_id,
            trace_id=root_trace_id,
        )
    ...
    emit_otel_span(
        ...
        get_time_nano(),
        parent_span_id=root_span_id,
        span_id=check6_span_id,
        trace_id=root_trace_id,
        attributes={
            OTelNames.ATTR_VALIDATION_LEVEL: "cgm_data",
            "failures": integrity_failures,
        },
        status_code="ERROR" if integrity_failures > 0 else "OK",
    )
    ...
    emit_otel_metric(
        ...
        OTelNames.METRIC_PASS_COUNT,
        passed_count,
        attributes={"check.category": OTelNames.CAT_CGM_INTEGRITY},
    )
    emit_otel_metric(
        emitter,
        otel_resource_id,
    ...
    emit_otel_metric(
        ...
        otel_resource_id,
        OTelNames.METRIC_VALIDATION_DURATION,
        (get_time_nano() - check6_start) / 1e9,
        unit="s",
        attributes={"check.category": OTelNames.CAT_CGM_INTEGRITY},
    )
    ...
    if os.path.exists(find_file(data_dir, FILES["meal_file_metadata"])):
        ...
        for res in meal_consistency_results:
            ...
            emit_otel_log(
                ...
                otel_severity,
                f"Diagnostic {res['status']}: {res['name']}",
                attributes={"check_id": 7, "details": res["details"]},
                span_id=current_span_id,
                trace_id=root_trace_id,
            )
        ...
        emit_otel_span(
            ...
            get_time_nano(),
            parent_span_id=root_span_id,
            span_id=check7_span_id,
            trace_id=root_trace_id,
            attributes={OTelNames.ATTR_VALIDATION_LEVEL: "meal_meta"},
            status_code="ERROR" if meal_cons_failures > 0 else "OK",
        )
        ...
        emit_otel_metric(
            ...
            OTelNames.METRIC_PASS_COUNT,
            passed_count,
            attributes={"check.category": OTelNames.CAT_MEAL_METADATA},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
        ...
        emit_otel_metric(
            ...
            otel_resource_id,
            OTelNames.METRIC_VALIDATION_DURATION,
            (get_time_nano() - check7_start) / 1e9,
            unit="s",
            attributes={"check.category": OTelNames.CAT_MEAL_METADATA},
        )
        ...
        if meal_cons_failures > 0:
            ...
            for res in meal_integrity_results:
                ...
                emit_otel_log(
                    ...
                    otel_severity,
                    f"Diagnostic {res['status']}: {res['name']}",
                    attributes={"check_id": 8, "details": res["details"]},
                    span_id=current_span_id,
                    trace_id=root_trace_id,
                )
            ...
            emit_otel_span(
                ...
                get_time_nano(),
                parent_span_id=root_span_id,
                span_id=check8_span_id,
                trace_id=root_trace_id,
                attributes={OTelNames.ATTR_VALIDATION_LEVEL: "meal_integrity"},
                status_code="ERROR" if meal_int_failures > 0 else "OK",
            )
            ...
            emit_otel_metric(
                ...
                OTelNames.METRIC_PASS_COUNT,
                passed_count,
                attributes={"check.category": OTelNames.CAT_MEAL_INTEGRITY},
            )
            emit_otel_metric(
                emitter,
                otel_resource_id,
            ...
            emit_otel_metric(
                ...
                otel_resource_id,
                OTelNames.METRIC_VALIDATION_DURATION,
                (get_time_nano() - check8_start) / 1e9,
                unit="s",
                attributes={"check.category": OTelNames.CAT_MEAL_INTEGRITY},
            )
    ...
    if os.path.exists(find_file(data_dir, FILES["fitness_file_metadata"])):
        ...
        for res in fitness_consistency_results:
            ...
            emit_otel_log(
                ...
                otel_severity,
                f"Diagnostic {res['status']}: {res['name']}",
                attributes={"check_id": 9, "details": res["details"]},
                span_id=current_span_id,
                trace_id=root_trace_id,
            )
        ...
        emit_otel_span(
            ...
            get_time_nano(),
            parent_span_id=root_span_id,
            span_id=check9_span_id,
            trace_id=root_trace_id,
            attributes={OTelNames.ATTR_VALIDATION_LEVEL: "fitness_meta"},
            status_code="ERROR" if fitness_cons_failures > 0 else "OK",
        )
        ...
        emit_otel_metric(
            ...
            OTelNames.METRIC_PASS_COUNT,
            passed_count,
            attributes={"check.category": OTelNames.CAT_FITNESS_METADATA},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_FAIL_COUNT,
            fitness_cons_failures,
            attributes={"check.category": OTelNames.CAT_FITNESS_METADATA},
        )
        emit_otel_metric(
            emitter,
            otel_resource_id,
            OTelNames.METRIC_VALIDATION_DURATION,
            (get_time_nano() - check9_start) / 1e9,
            unit="s",
            attributes={"check.category": OTelNames.CAT_FITNESS_METADATA},
        )
        ...
        if fitness_cons_failures > 0:
            ...
            for res in fitness_integrity_results:
                ...
                emit_otel_log(
                    ...
                    otel_severity,
                    f"Diagnostic {res['status']}: {res['name']}",
                    attributes={"check_id": 10, "details": res["details"]},
                    span_id=current_span_id,
                    trace_id=root_trace_id,
                )
            ...
            emit_otel_span(
                ...
                get_time_nano(),
                parent_span_id=root_span_id,
                span_id=check10_span_id,
                trace_id=root_trace_id,
                attributes={OTelNames.ATTR_VALIDATION_LEVEL: "fitness_integrity"},
                status_code="ERROR" if fitness_int_failures > 0 else "OK",
            )
            ...
            emit_otel_metric(
                ...
                OTelNames.METRIC_PASS_COUNT,
                passed_count,
                attributes={"check.category": OTelNames.CAT_FITNESS_INTEGRITY},
            )
            emit_otel_metric(
                emitter,
                otel_resource_id,
                OTelNames.METRIC_FAIL_COUNT,
                fitness_int_failures,
                attributes={"check.category": OTelNames.CAT_FITNESS_INTEGRITY},
            )
            emit_otel_metric(
                emitter,
                otel_resource_id,
                OTelNames.METRIC_VALIDATION_DURATION,
                (get_time_nano() - check10_start) / 1e9,
                unit="s",
                attributes={"check.category": OTelNames.CAT_FITNESS_INTEGRITY},
            )
    ...
    if overall_status == "FAILED":
        ...
        LOGGER.error(
            "Validation failed in Meal or Fitness checks. See drh_diagnostics for details."
        )
        emit_otel_log(
            emitter,
            otel_resource_id,
            "ERROR",
            "Validation failed (Meal/Fitness Phase)",
            {"folder": data_dir},
        )
    ...
    filename_to_stream = {
        v: k for k, v in FILES.items() if k not in ("meal_data", "fitness_data")
    }
    ...
    for filepath in glob.glob(os.path.join(data_dir, "**", "*.csv"), recursive=True):
        ...
        if filename in filename_to_stream:
            ...
            if processor:
                ...
                LOGGER.warning(
                    f"No processor defined for stream {stream_name} (file: {filename})"
                )
        ...
        emit_otel_log(
            emitter, otel_resource_id, "WARN", f"Skipping unknown file type: {filename}"
        )

    # Emit Metrics
    emit_otel_metric(
        emitter, otel_resource_id, "files_processed", files_processed_count, "1"
    )

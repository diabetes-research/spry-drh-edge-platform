...
def check_file_headers(data_dir):
    ...
    for key, expected_cols in EXPECTED_HEADERS.items():
        ...
        if os.path.exists(fpath):
            with tracer.start_as_current_span(f"{OTelNames.CHK_FILE_VAL}: {fname}") as file_span:
                file_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "schema")
                file_span.set_attribute(OTelNames.ATTR_FILE_NAME, fname)
                ...
                try:
                    with open(fpath, 'r', encoding='utf-8-sig') as f:
                        ...
                        try:
                            ...
                            with tracer.start_as_current_span(f"{OTelNames.CHK_COLUMNS}: {fname}") as req_cols_span:
                                req_cols_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "column_check")
                                req_cols_span.set_attribute(OTelNames.ATTR_FILE_NAME, fname)
                                ...
                                if missing_cols:
                                    ...
                                    req_cols_span.set_attribute("missing", str(missing_cols))
                                    req_cols_span.set_status(Status(StatusCode.ERROR))
                                    
                                    file_span.set_attribute("error", "Missing Columns")
                                    ...
                                    req_cols_span.set_status(Status(StatusCode.OK))
                            ...
                            # Emit Type and Pattern Spans concurrently since they are processed in loop
                            sub_status = StatusCode.ERROR if row_errors else StatusCode.OK
                            
                            with tracer.start_as_current_span(f"{OTelNames.CHK_TYPE}: {fname}") as type_span:
                                type_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "type_check")
                                if row_errors: type_span.set_attribute("error_count", len(row_errors))
                                type_span.set_status(Status(sub_status))

                            with tracer.start_as_current_span(f"{OTelNames.CHK_PATTERNS}: {fname}") as pattern_span:
                                pattern_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "pattern_check")
                                if row_errors: pattern_span.set_attribute("error_count", len(row_errors))
                                pattern_span.set_status(Status(sub_status))
                            ...
                            if row_errors:
                                ...
                                file_span.set_attribute(OTelNames.ATTR_ERROR_COUNT, len(row_errors))
                                file_span.set_status(Status(StatusCode.ERROR))
                            else:
                                results.append({
                                    "name": f"File Schema Check: {fname}",
                            ...
                            with tracer.start_as_current_span(f"{OTelNames.CHK_COLUMNS}: {fname}") as req_cols_span:
                                req_cols_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "column_check")
                                req_cols_span.set_attribute("error", "Empty File")
                                req_cols_span.set_status(Status(StatusCode.ERROR))
                            ...
                            file_span.set_attribute("error", "Empty File")
                            file_span.set_status(Status(StatusCode.ERROR))
                    ...
                    file_span.set_attribute("error", str(e))
                    file_span.set_status(Status(StatusCode.ERROR))
...
def check_cgm_metadata_consistency(data_dir):
    ...
    if os.path.exists(cgm_meta_path):
        with tracer.start_as_current_span(OTelNames.CAT_CGM_TRACE_METADATA) as metadata_span:
            metadata_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "metadata_check")
            try:
                    ...
                    for row in reader:
                        ...
                        if expected_file:
                            ...
                            if os.path.exists(target_path):
                                ...
                                metadata_span.set_attribute("missing_file", expected_file)
                                metadata_span.set_status(Status(StatusCode.ERROR))
                                results.append({
                                    "name": check_name,
                                    "status": "FAILED",
                ...
                metadata_span.set_attribute("error", str(e))
                metadata_span.set_status(Status(StatusCode.ERROR))
                results.append({
...
def check_cgm_data_integrity(data_dir):
    ...
    with tracer.start_as_current_span(OTelNames.CAT_CGM_INTEGRITY) as integrity_span:
        integrity_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "integrity_check")
        try:
            with open(cgm_meta_path, 'r', encoding='utf-8-sig') as f:
                reader = csv.DictReader(f)
                ...
                if "file_name" not in reader.fieldnames:
                    integrity_span.set_status(Status(StatusCode.ERROR))
                    return [{
                ...
                for row in rows:
                    ...
                    if expected_file:
                        ...
                        if os.path.exists(target_path):
                            ...
                            try:
                                with open(target_path, 'r', encoding='utf-8-sig') as data_file:
                                    ...
                                    try:
                                        ...
                                        integrity_span.set_attribute("empty_file", expected_file_with_ext)
                                        integrity_span.set_status(Status(StatusCode.ERROR))
                                        results.append({
                                            "name": check_name,
                                            "status": "FAILED",
                                    ...
                                    try:
                                        ...
                                        integrity_span.set_attribute("no_data", expected_file_with_ext)
                                        integrity_span.set_status(Status(StatusCode.ERROR))
                                        results.append({
                                            "name": check_name,
                                            "status": "FAILED",
                                ...
                                integrity_span.set_attribute("error", str(e))
                                integrity_span.set_status(Status(StatusCode.ERROR))
                                results.append({
                                    "name": check_name,
                                    "status": "FAILED",
                            ...
                            integrity_span.set_attribute("missing_file", expected_file_with_ext)
                            integrity_span.set_status(Status(StatusCode.ERROR))
                            results.append({
                                "name": check_name,
                                "status": "FAILED",
...
def check_meal_data_consistency(data_dir):
    ...
    if os.path.exists(meal_meta_path):
        with tracer.start_as_current_span(OTelNames.CAT_MEAL_METADATA) as metadata_span:
            metadata_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "metadata_check")
            try:
...
def check_fitness_data_consistency(data_dir):
    ...
    if os.path.exists(fitness_meta_path):
        with tracer.start_as_current_span(OTelNames.CAT_FITNESS_METADATA) as metadata_span:
            metadata_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "metadata_check")
            try:
...
def check_meal_data_integrity(data_dir):
    ...
    if not os.path.exists(meal_meta_path):
        return []
        
    with tracer.start_as_current_span(OTelNames.CAT_MEAL_INTEGRITY) as integrity_span:
        integrity_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "integrity_check")
        try:
             with open(meal_meta_path, 'r', encoding='utf-8-sig') as f:
                reader = csv.DictReader(f)
...
def check_fitness_data_integrity(data_dir):
    ...
    if not os.path.exists(fitness_meta_path):
        return []
        
    with tracer.start_as_current_span(OTelNames.CAT_FITNESS_INTEGRITY) as integrity_span:
        integrity_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "integrity_check")
        try:
             with open(fitness_meta_path, 'r', encoding='utf-8-sig') as f:
                reader = csv.DictReader(f)
...
def main():
    ...
    # Start Root Span
    with tracer.start_as_current_span(OTelNames.ROOT_VV) as root_span:
        ...
        with tracer.start_as_current_span(OTelNames.CAT_FOLDER_SCAN) as folder_span:
            folder_val_start = get_time_nano()
            folder_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "folder")
            folder_span.set_attribute("folder.path", data_dir)
        
            if not os.path.exists(data_dir):
                msg = f"Data directory does not exist: {data_dir}"
                emit_diagnostic(1, "Folder & Resource Check", "FAILED", msg)
                emit_otel_log(emitter, otel_resource_id, "ERROR", f"Diagnostic FAILED: Folder & Resource Check", attributes={"check_id": 1, "details": msg})
                folder_span.set_status(Status(StatusCode.ERROR))
                ...
                emit_otel_log(emitter, otel_resource_id, "INFO", f"Diagnostic PASSED: Folder & Resource Check", attributes={"check_id": 1, "details": msg})
                folder_span.set_attribute("files.found", file_count)
        ...
        with tracer.start_as_current_span(OTelNames.CAT_MANDATORY_FILES) as mandatory_span:
            check2_start = get_time_nano()
            mandatory_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "file_presence")
            ...
            if missing:
                ...
                emit_otel_log(emitter, otel_resource_id, "ERROR", f"Diagnostic FAILED: Mandatory File Presence", attributes={"check_id": 2, "details": msg})
                mandatory_span.set_attribute("missing", str(missing))
                mandatory_span.set_status(Status(StatusCode.ERROR))
                ...
                emit_otel_log(emitter, otel_resource_id, "INFO", f"Diagnostic PASSED: Mandatory File Presence", attributes={"check_id": 2, "details": "All mandatory files present"})
        ...
        with tracer.start_as_current_span(OTelNames.CAT_EXTENSION_CHECK) as ext_span:
            check3_start = get_time_nano()
            ext_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "extensions")
            ...
            if extension_errors:
                for err in extension_errors:
                     ...
                     emit_otel_log(emitter, otel_resource_id, "ERROR", f"Diagnostic FAILED: File Extension Validation", attributes={"check_id": 3, "details": err})
            
                ext_span.set_attribute(OTelNames.ATTR_ERROR_COUNT, len(extension_errors))
                ext_span.set_status(Status(StatusCode.ERROR))
                ...
                fail_and_exit("File Extension Validation", len(extension_errors))
                 ...
                 emit_otel_log(emitter, otel_resource_id, "INFO", f"Diagnostic PASSED: File Extension Validation", attributes={"check_id": 3, "details": msg})


        # 4. File Schema and Required Columns + Field Format Check (Static ID: 4)
        with tracer.start_as_current_span(OTelNames.CAT_SCHEMA_VALIDATION) as schema_span:
            schema_val_start = get_time_nano()
            schema_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "schema")
            ...
            schema_span.set_attribute("files.checked", len(schema_results))
            if schema_failures > 0:
                schema_span.set_attribute("failures", schema_failures)
        ...
        with tracer.start_as_current_span(OTelNames.CAT_CGM_TRACE_METADATA) as meta_span:
            check5_start = get_time_nano()
            meta_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "cgm_meta")
            ...
            if consistency_failures > 0:
                meta_span.set_attribute("failures", consistency_failures)
                meta_span.set_status(Status(StatusCode.ERROR))
        ...
        with tracer.start_as_current_span(OTelNames.CAT_CGM_INTEGRITY) as integ_span:
            check6_start = get_time_nano()
            integ_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "cgm_data")
            ...
            if integrity_failures > 0:
                integ_span.set_attribute("failures", integrity_failures)
                integ_span.set_status(Status(StatusCode.ERROR))
        ...
        if os.path.exists(find_file(data_dir, FILES["meal_file_metadata"])):
            # Check 7
            with tracer.start_as_current_span(OTelNames.CAT_MEAL_METADATA) as meal_meta_span:
                check7_start = get_time_nano()
                meal_meta_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "meal_meta")
                ...
                if meal_cons_failures > 0:
                    meal_meta_span.set_status(Status(StatusCode.ERROR))
            ...
            if meal_cons_failures > 0:
                 ...
                 with tracer.start_as_current_span(OTelNames.CAT_MEAL_INTEGRITY) as meal_integ_span:
                     check8_start = get_time_nano()
                     meal_integ_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "meal_integrity")
                     ...
                     if meal_int_failures > 0:
                         meal_integ_span.set_status(Status(StatusCode.ERROR))
        ...
        if os.path.exists(find_file(data_dir, FILES["fitness_file_metadata"])):
            # Check 9
            with tracer.start_as_current_span(OTelNames.CAT_FITNESS_METADATA) as fit_meta_span:
                check9_start = get_time_nano()
                fit_meta_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "fitness_meta")
                ...
                if fitness_cons_failures > 0:
                    fit_meta_span.set_status(Status(StatusCode.ERROR))
            ...
            if fitness_cons_failures > 0:
                 ...
                 with tracer.start_as_current_span(OTelNames.CAT_FITNESS_INTEGRITY) as fit_integ_span:
                     check10_start = get_time_nano()
                     fit_integ_span.set_attribute(OTelNames.ATTR_VALIDATION_LEVEL, "fitness_integrity")
                     ...
                     if fitness_int_failures > 0:
                          fit_integ_span.set_status(Status(StatusCode.ERROR))
        ...
        if overall_status == "FAILED":
             ...
             # Emit Failure Span explicitly since sys.exit finishes the span
             root_span.set_status(Status(StatusCode.ERROR))
             sys.exit(0)

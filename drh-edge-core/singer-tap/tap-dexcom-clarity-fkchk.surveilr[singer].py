...
def get_stream_keys():
    ...
    for stream in ALL_STREAM_NAMES:
        ...
        if schema and "key_properties" in schema:
             ...
             if schema and "properties" in schema and default_key in schema["properties"]:
...
class ForeignKeyValidator:
    ...
    def validate(self):
        ...
        """
        violations = []
        print(f"INFO:__main__:Starting Global FK Validation on {len(self.pending_fks)} references across {len(self.stream_ids)} streams...", file=sys.stderr)
        ...
        for check in self.pending_fks:
            ...
            if target_stream not in self.stream_ids:
                ...
                violations.append(f"FK Violation in {check['source_file']} line {check['line']}: {check['field']}='{val}' refers to missing stream '{target_stream}'")
                continue
                
            if val not in self.stream_ids[target_stream]:
                violations.append(f"FK Violation in {check['source_file']} line {check['line']}: {check['field']}='{val}' not found in {target_stream}")

...
- The YAML header at the top of this `drh-spry-partial-test.md` reads `database_url: ${env.SPRY_DB}` and `port: ${env.PORT}` — Spry and the SQLPage tooling will substitute those environment values when building or serving the site.
...
whenever you update `drh-spry-partial-test.md`, it regenerates the SQLPage `dev-src.auto`,

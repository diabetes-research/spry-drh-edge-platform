...
export PORT=9227
export STUDY_DATA_PATH="raw-data/simplera-cgm-systems/"
...
- The YAML header at the top of this `drh-simplera-cgm-systems.md` reads `database_url: ${env.SPRY_DB}` and `port: ${env.PORT}` — Spry and the SQLPage tooling will substitute those environment values when building or serving the site.
...
- Place the study data files in a **directory** in the same path as this markdown, then run the following command:
  - `spry rb task prepare-db-deploy-server  drh-simplera-cgm-systems.md`
...
surveilr ingest files -r singer-tap/tap-simplera-cgm-systems.surveilr\[singer]\.py 
...
spry sp spc --package --conf sqlpage/sqlpage.json -m drh-simplera-cgm-systems.md | sqlite3 resource-surveillance.sqlite.db
```
